	...
	const std::string cloud_detection_wf = R"WF_ESCAPE( 
{
  ...
  "sources": {
	...
	const QuerySpec btw(btw_wf,CrsId::from_epsg_code(4326),CacheType::RASTER,TemporalReference(TIMETYPE_UNIX,42), "BTW Cats");
	const QuerySpec cloud_detection(cloud_detection_wf,CrsId::from_epsg_code(0x9E05),CacheType::RASTER,tref, "CloudDetection");
	const QuerySpec avg_temp(avg_temp_wf,CrsId::from_epsg_code(4326),CacheType::RASTER,tref, "Average Temperature");
	const QuerySpec srtm_proj(srtm_proj_wf,CrsId::from_epsg_code(3857),CacheType::RASTER,tref, "SRTM Projected");
	const QuerySpec srtm(srtm_wf,CrsId::from_epsg_code(4326),CacheType::RASTER,tref, "SRTM");
	const QuerySpec ndvi(ndvi_wf,CrsId::from_epsg_code(4326),CacheType::RASTER,tref_ndvi, "NDVI");
	const QuerySpec ndvi_proj(ndvi_proj_wf,CrsId::from_epsg_code(3857),CacheType::RASTER,tref_ndvi, "NDVI projected");
	const QuerySpec srtm_ts(srtm_ts_wf,CrsId::from_epsg_code(4326),CacheType::RASTER,tref, "SRTM Timeshifted");
	const QuerySpec srtm_ex(srtm_ex_wf,CrsId::from_epsg_code(4326),CacheType::RASTER,tref, "SRTM Expression");
	...
	static QuerySpec projected_shifted_temp( const std::string &timestamp, const std::string &time_to ) {
		...
		return QuerySpec( wf, CrsId::from_epsg_code(3857), CacheType::RASTER,tref,"Monthly Temperature (Projected, Shifted to " + timestamp + ")" );
	}
	...
	static QuerySpec shifted_temp( const std::string &timestamp, const std::string &time_to ) {
		...
		return QuerySpec( wf, CrsId::from_epsg_code(4326), CacheType::RASTER,tref,"Monthly Temperature (Shifted to " + timestamp + ")" );
	}
}

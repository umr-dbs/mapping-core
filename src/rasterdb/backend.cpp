...
typedef std::unique_ptr<RasterDBBackend> (*BackendConstructor)(const std::string &location, const ConfigurationTable& params);
...
std::unique_ptr<RasterDBBackend> RasterDBBackend::create(const std::string &backend, const std::string &location, const ConfigurationTable& params) {
	auto map = getRegisteredConstructorsMap();
